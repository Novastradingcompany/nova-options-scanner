    ...
    if st.button("Scan") and expiry_selected:
        with st.spinner("🔍 Scanning options..."):
            ...
            if spread_type == "Bull Put":
                trades, rejects = scan_verticals(opt_chain.puts, spot_price, expiry_selected, dte, T, max_width, max_loss, min_pop, raw_mode, "put", contracts)
            elif spread_type == "Bear Call":
                trades, rejects = scan_verticals(opt_chain.calls, spot_price, expiry_selected, dte, T, max_width, max_loss, min_pop, raw_mode, "call", contracts)
            else:
                trades = scan_condors(opt_chain, spot_price, expiry_selected, dte, T, max_width, max_loss, min_pop, raw_mode, contracts)
                rejects = []
